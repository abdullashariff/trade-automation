...
if not mt5.initialize():
    print("Failed to initialize MetaTrader 5. Please check if MetaTrader 5 terminal is running.")
    mt5.shutdown()
    exit()  # Exit the script if MetaTrader 5 initialization fails
else:
...
def fetch_real_time_data(symbol):
    ...
    if prices is None:
        print(f"Failed to fetch market data for {symbol}. Please ensure the symbol is available in Market Watch.")
        return None
    print(f"Fetched market data for {symbol}: Ask={prices.ask}, Bid={prices.bid}")
...
def determine_leverage(atr):
    if atr > volatility_threshold_high:
        print("Market is highly volatile. Using low leverage (1:10).")
        return 10  # Low leverage for high volatility
    elif atr > volatility_threshold_moderate:
        print("Market is moderately volatile. Using moderate leverage (1:25).")
        return 25  # Moderate leverage for moderate volatility
    else:
        print("Market is calm (sideways). Using high leverage (1:50).")
        return 50  # High leverage for low volatility
...
def trailing_stop_loss(symbol, order_ticket):
    while True:
       ...
       # elif order_info.type == mt5.ORDER
